      ...
      <div class="flex items-start gap-3">
        ...
        <div class="flex-1">
          ...
          <p class="text-sm text-brand-primary mb-4 sm:mb-3">
            We detected an issue with your login session. You're viewing the getting started page in guest mode. 
...
onMounted(async () => {
  checkInsidersAccess()
  console.log('Getting Started page mounted, insiders access:', hasInsidersAccess.value)
...
watchEffect(async () => {
  console.log('[Getting Started] watchEffect user:', user)
  console.log('[Getting Started] watchEffect user.value:', user?.value)
  
  if (user.value && !isGuestMode.value) {
    console.log('[Getting Started] User detected, checking if new user...')
...
async function checkIfNewUser() {
  try {
    console.log('[Getting Started] checkIfNewUser: Starting check...')
    const db = useDatabase()
    
    // Check for assets
    console.log('[Getting Started] checkIfNewUser: Checking assets...')
    const assetsResponse = await db.assets.getAssets()
    hasAssets.value = assetsResponse && assetsResponse.length > 0
    console.log('[Getting Started] checkIfNewUser: Assets response:', assetsResponse)
    console.log('[Getting Started] checkIfNewUser: hasAssets:', hasAssets.value)
    
    // Check for memory books
    console.log('[Getting Started] checkIfNewUser: Checking memory books...')
    const booksResponse = await db.memoryBooks.getMemoryBooks()
    memoryBooks.value = booksResponse || []
    console.log('[Getting Started] checkIfNewUser: Memory books response:', booksResponse)
    console.log('[Getting Started] checkIfNewUser: memoryBooks length:', memoryBooks.value.length)
    
    // Show Savta bubble if user has no memory books and no assets
    console.log('[Getting Started] checkIfNewUser: Checking conditions - memoryBooks.length === 0:', memoryBooks.value.length === 0, '!hasAssets:', !hasAssets.value)
    if (memoryBooks.value.length === 0 && !hasAssets.value) {
      console.log('[Getting Started] New user detected, showing Savta bubble')
      // Functionality moved to memory-books/index.vue
      // showSavtaBubble.value = true
      console.log('[Getting Started] showSavtaBubble set to:', showSavtaBubble.value)
    } else {
      console.log('[Getting Started] User is not new, not showing Savta bubble')
    }
  } catch (error) {
    console.error('[Getting Started] Error checking if new user:', error)
  }
}
...
async function handleGetStarted() {
  console.log('[Getting Started] handleGetStarted user:', user)
  console.log('[Getting Started] handleGetStarted user.value:', user.value)
  if (user.value) {
    console.log('[Getting Started] Navigating to /app/memory-books/upload')
    await navigateTo('/app/memory-books/upload')
    console.log('[Getting Started] Navigation to /app/memory-books/upload complete')
  } else {
    console.log('[Getting Started] Navigating to /app/login')
    await navigateTo('/app/login')
    console.log('[Getting Started] Navigation to /app/login complete')
  }
}

function handleCardClick(cardType) {
  console.log('[Getting Started] Card clicked:', cardType, 'User:', user.value)
