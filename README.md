...
For displaying PDFs in the browser, we use [`@vue-pdf-viewer/viewer`](https://www.npmjs.com/package/@vue-pdf-viewer/viewer), a Vue 3 component for rendering and interacting with PDF files. This library is used **only for viewing** PDFs generated by the backend (`pdf-lib`).

- Provides a modern, mobile-friendly PDF viewer UI for users
- Supports zoom, search, thumbnails, print, and download
- Integrates seamlessly with Vue 3 and Nuxt 3
- Does not generate or modify PDFs—only displays them

**Summary:**
- **PDF Generation:** `pdf-lib` (backend, Node.js)
- **PDF Viewing:** `@vue-pdf-viewer/viewer` (frontend, Vue 3)